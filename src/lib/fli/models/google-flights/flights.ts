 ...
 * must remain byte-identical to the Python implementation.
 */
...
function airlineSortKey(a: Airline): string {
  const code = serializeCode(a as string);
  ...
  format(): unknown[] {
    ...
    for (let segIdx = 0; segIdx < this.flight_segments.length; segIdx++) {
      ...
      if (this.airlines?.length) {
        ...
        for (const a of sorted) includeTokens.push(serializeCode(a as string));
      }
      ...
      if (this.airlines_exclude?.length) {
        ...
        for (const a of sorted) excludeTokens.push(serializeCode(a as string));
      }
      ...
      if (isMultiLeg && segment.selected_flight != null) {
        selectedFlights = segment.selected_flight.legs.map((leg) => [
          serializeCode(leg.departure_airport as string),
          formatDateOnly(leg.departure_datetime),
          serializeCode(leg.arrival_airport as string),
          null,
          serializeCode(leg.airline as string),
          leg.flight_number,
        ]);
      }
