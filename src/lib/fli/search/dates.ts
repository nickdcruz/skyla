...
import { DateSearchFilters } from "../models/google-flights/dates";
...
export class SearchDates {
  ...
  async search(
    ...
    const chunkResults = await Promise.all(chunkFilters.map((cf) => this._searchChunk(cf, options)));
  ...
  static _parseDate(item: unknown, tripType: number): [Date] | [Date, Date] {
